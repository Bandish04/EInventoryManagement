...
const AuthScreen = () => {
  ...
  const doSignup = async(e) => {
    e.preventDefault();
    let response=await callApi({url:"http://127.0.0.1:8000/api/auth/signup/",method:"POST",body:{username:e.target.username.value,password:e.target.password.value,email:e.target.email.value,profile_pic:"https://www.google.com/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png"}});
...
const doLogin = async(e) => {
    e.preventDefault();
    let response=await callApi({url:"http://127.0.0.1:8000/api/auth/login/",method:"POST",body:{username:e.target.username.value,password:e.target.password.value}});
    console.log(response);
    if(response?.data?.access){
      localStorage.setItem("token",response.data.access);
          ...
          <Card sx={{ maxWidth: 400, width: '100%' }}>
            <CardContent>
              <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
                <img src="/logo.svg" alt="Logo" style={{ borderRadius: '50%',width:'100px' }} />
              </Box>
            ...
            <Box sx={{ textAlign: 'center', py: 2, borderTop: '1px solid', borderColor: theme.palette.divider }}>
              <Typography variant="body2" color="text.secondary">
                © 2026 Bandish Vaidya. All rights reserved.
              </Typography>
            </Box>
          </Card>
        </Box>
